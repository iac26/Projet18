...
static int single_collisions(	C2D _this, double angle, double tran, double rot, 
	...
	for(int i = 0; i < nb_robot; i++){
		single_collisions_robot(_this, new_this, &tran, u_id);
		new_this.centre = util_deplacement(_this.centre, angle, tran);
		robot_increment();
	}
	particle_get_init_head();
	for(int i = 0; i < nb_particle; i++){
		single_collisions_particle(	_this, new_this, angle, &tran, &eaten);
		new_this.centre = util_deplacement(_this.centre, angle, tran);
		particle_increment();
	}
	robot_pop_last();
...
static void simulation_move(void){
	...
	S2D targ;
	unsigned int id;
	double c_angle, v_tran = 0, v_rot = 0, delta_angle;
	int selected;
		...
		if(util_ecart_angle(c_robot.centre, c_angle, targ, &delta_angle)){
			if(fabs(delta_angle) < OPTIMAL_ANGLE){
				if(VTRAN_MAX*DELTA_T > util_distance(c_robot.centre, targ)){
					v_tran = util_distance(c_robot.centre, targ)/DELTA_T;
				} else{
					v_tran = VTRAN_MAX;
				}
			}
			if(fabs(delta_angle) < VROT_MAX*DELTA_T){
				v_rot = delta_angle/DELTA_T;
			} else{
				if(delta_angle < 0){
					v_rot = -VROT_MAX;
				} else{
...
static void select_in_quads(void){
	int nb_robot = robot_get_nb(), nb_part = particle_get_nb();
	robot_block_increment();
	particle_block_increment();
...
void simulation_assign_quads(void){
	robot_block_increment();
	int nb_robot = robot_get_nb(), rob_per_quad = nb_robot/4;
	robot_get_init_head();
	S2D rob;
